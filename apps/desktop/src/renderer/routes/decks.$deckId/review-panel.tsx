...
import type { CardWithNote } from "@orbit/types";
...
import { loadAnkiPreferences, parseAnswerKeyPreference } from "@/lib/anki-preferences";
import { useDueCardsQuery } from "@/lib/queries/review";
import {
...
export function ReviewPanel({ deckId, onCreateCopy, onFinished }: ReviewPanelProps) {
  const { data: [dueCardsPage] = [], refresh: refreshDueCards } = useDueCardsQuery({ deckId });
  const currentCard = dueCardsPage?.data[0];
  ...
  const [audioInterruptMessage, setAudioInterruptMessage] = useState("");
  const [ownVoice, setOwnVoice] = useState<OwnVoiceState>();
  const [preferences] = useState(() => loadAnkiPreferences());
  ...
  const typedAnswerComparison =
    side === "answer" && typedAnswerPrompt?.target
      ...
      : undefined;
  const currentCardHasAudio =
    currentCard?.front.includes("[sound:") || currentCard?.back.includes("[sound:");
  const currentCardHasLatex =
    currentCard?.front.includes("\\(") ||
    currentCard?.front.includes("\\[") ||
  ...
  useEffect(() => {
    if (!currentCard || !isAutoAdvanceEnabled || side !== "answer" || isReviewing) {
      return undefined;
    }
  ...
  async function advanceAfterQueueMutation() {
    ...
    if ((dueCardsPage?.data.length ?? 0) <= 1) {
      onFinished?.();
    }
  }
  ...
  function submitRating(value: 1 | 2 | 3 | 4 | 5) {
    ...
    setIsReviewing(true);
    void submitReview(reviewedCard.id, {
      elapsedMilliseconds,
      value,
    })
      .then(() => {
  ...
  function updateCurrentCard(input: {
    ...
    void updateCard(currentCard.id, input)
      .then(() => {
  ...
  function updateCurrentNote(input: { buried?: boolean; marked?: boolean; suspended?: boolean }) {
    ...
    void updateNote(currentCard.noteId, input)
      .then(() => {
  ...
  function deleteCurrentNote() {
    ...
    void deleteNote(currentCard.noteId)
      .then(() => {
  ...
  function revealCurrentAnswer() {
    ...
    if (preferences.interruptAudioOnAnswer && reviewAudio?.status === "playing") {
      setAudioInterruptMessage("Audio interrupted");
    }
  ...
  function saveCurrentNoteEdit() {
    ...
    setIsUpdatingNote(true);
    void updateNote(currentCard.noteId, editNoteDraft)
      .then(() => {
  ...
  useEffect(() => {
    if (!currentCard || side !== "answer" || isReviewing) {
      return undefined;
    }

    const answerKeyMap = parseAnswerKeyPreference(preferences.answerKeys);
    const submitRatingByLabel = (label: string) => {
      const targetRating = ratingButtons.find((rating) => rating.label === label);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === " " && preferences.spacebarRatesCard) {
        event.preventDefault();
        submitRatingByLabel("Good");
        return;
      }

      const configuredLabel = answerKeyMap[event.key];

      if (configuredLabel) {
        event.preventDefault();
        submitRatingByLabel(configuredLabel);
      }
    };
    ...
    currentCard?.id,
    preferences.answerKeys,
    preferences.spacebarRatesCard,
    ratingButtons,
    isReviewing,
    side,
  ]);
  ...
  return (
    <section
      className="min-w-0"
      data-minimalist={String(preferences.minimalistMode)}
    ...
    >
      <div
        ...
        hidden={preferences.hideTopBarDuringReview}
      >
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
            <span>Scheduler day starts at {preferences.nextDayStartsAt}:00</span>
            {preferences.learnAheadMinutes > 0 ? (
              <span>Learn ahead {preferences.learnAheadMinutes}m</span>
            ) : null}
            {preferences.timeboxMinutes > 0 ? (
              <span>Timebox {preferences.timeboxMinutes}m</span>
            ) : null}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {preferences.showRemainingCardCount ? (
            <Badge aria-label="Remaining card count" variant="outline">
              {dueCardsPage?.pagination.total ?? 0} due
            </Badge>
          ) : null}
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                disabled={!currentCard || isUpdatingCard}
                size="icon-sm"
                type="button"
                variant="outline"
              >
      ...
      {!currentCard ? (
        ...
        <Card>
          ...
          <CardContent className="grid gap-4">
            ...
            {audioInterruptMessage ? (
              <p className="rounded-md border border-border bg-muted p-3 text-sm text-muted-foreground">
                {audioInterruptMessage}
              </p>
            ) : null}
            {preferences.showAudioPlayButtons && currentCardHasAudio ? (
              <Button onClick={replayCurrentAudio} type="button" variant="outline">
                Play card audio
              </Button>
            ) : null}
            {preferences.generateLatexImages && currentCardHasLatex ? (
              <p className="rounded-md border border-border bg-muted p-3 text-sm text-muted-foreground">
                LaTeX images generated
              </p>
            ) : null}
            {currentCardOwnVoice ? (
              <p
            ...
            <div data-testid="review-bottom-bar" hidden={preferences.hideBottomBarDuringReview}>
              {side === "question" ? (
                ...
                <div className="grid gap-2">
                  {preferences.showNextReviewTime ? (
                    <p className="text-sm text-muted-foreground">
                      Next review: Again 1m · Hard 6m · Good 1d · Easy 4d
                    </p>
                  ) : null}
      ...
      {currentCard && isEditNoteOpen ? (
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              disabled={isUpdatingNote || !editNoteDraft.front.trim() || !editNoteDraft.back.trim()}
      ...
      {currentCard && isForgetCardOpen ? (
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button disabled={isUpdatingCard} onClick={forgetCurrentCard} type="button">
              Forget card
            </Button>
          </div>
        </section>
      ...
      {currentCard && isDueDateOpen ? (
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              disabled={!dueDateInput || isUpdatingCard}
