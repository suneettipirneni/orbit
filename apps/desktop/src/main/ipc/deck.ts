...
export function registerDeckIpcHandlers(repositories: Repositories) {
  ipcMain.handle("orbit:decks:list", (_event, input: PaginationInput | undefined) =>
    repositories.listDecks(input),
  );

  ipcMain.handle("orbit:decks:create", (_event, input) =>
    repositories.createDeck(createDeckInputSchema.parse(input)),
  );

  ipcMain.handle("orbit:decks:get", (_event, deckId: string) =>
    requireFound(repositories.getDeck(deckId), "Deck not found."),
  );

  ipcMain.handle("orbit:decks:update", (_event, deckId: string, input) =>
    requireFound(
      repositories.updateDeck(deckId, updateDeckInputSchema.parse(input)),
      "Deck not found.",
    ),
  );

  ipcMain.handle("orbit:decks:delete", (_event, deckId: string) => repositories.deleteDeck(deckId));

  ipcMain.handle("orbit:decks:import-anki", (_event, input: IpcImportAnkiDecksInput) =>
    importAnkiDecks(repositories, input),
  );

  ipcMain.handle(
    "orbit:decks:list-cards",
    (_event, deckId: string, input: ListDeckCardsInput | undefined) =>
      requireFound(repositories.listDeckCards(deckId, input), "Deck not found."),
  );

  ipcMain.handle(
    "orbit:decks:list-note-types",
    (_event, deckId: string, input: PaginationInput | undefined) =>
      requireFound(repositories.listDeckNoteTypes(deckId, input), "Deck not found."),
  );

  ipcMain.handle(
    "orbit:decks:list-card-types",
    (_event, deckId: string, input: PaginationInput | undefined) =>
      requireFound(repositories.listDeckCardTypes(deckId, input), "Deck not found."),
  );
}

function importAnkiDecks(repositories: Repositories, input: IpcImportAnkiDecksInput) {
  ...
  try {
    ...
    const result = repositories.importAnkiDecks(loadAnkiPackage(filePath));
